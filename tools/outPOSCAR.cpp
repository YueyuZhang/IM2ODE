...
//			printf("%.6f %.6f\n", ss[cnt].a, ss[cnt].b);
	...
	freopen("out.dat", "w", stdout);
		...
		printf("Frontier\tEnergy\tFitness\n");
	...
	while(flag){
		...
		for(i = 0; i < cnt; i ++){
			if(f1[i] == 0 && ss[i].b < gap){
				...
				gap = ss[i].b;
                if(tag == 2)
					printf("%d\t%d\t%.6f\t%.6f\n", front, num, ss[i].a, ss[i].b);
				else
					printf("%d\t%.6f\t%.6f\n", front, ss[i].a, ss[i].b);
				sprintf(strold, "POSCAR-%d", ss[i].c);
				if(tag == 2)
					sprintf(strnew, "POSCAR_%d_%d", front, num);
                                else
					sprintf(strnew,"POSCAR_%d",front);
